...
lemma assertions_inclusion:"pre c \<in> assertions c"
by (induct c) auto
...
lemma interfree_step_post:
 ...
 proof(cases opt)
  ...
  next
  case (Some u)
     have 1:" pre u s" using Some assms(3) by simp
     have 2: "(\<forall>(R, r) \<in> (atomics c). \<Turnstile>\<^sub>t\<^sub>r {\<lambda>s. Q s \<and> R s} r {Q} \<and> 
